...
public class BoxASTNodeLocator implements ISourcePositionLocator {
	...
	private final class NodeVisitor extends AbstractVisitor {
		...
		public boolean preVisit(ASTNode element) {
			...
			//System.out.println("BoxNodeLocator.NodeVisitor.preVisit(ASTNode):  Examining " + element.getClass().getName() +
			...
			if (nodeStartOffset <= fStartOffset && nodeEndOffset >= fEndOffset) {
				//System.out.println("BoxNodeLocator.NodeVisitor.preVisit(ASTNode) SELECTED for offsets [" + fStartOffset + ".." + fEndOffset + "]");
				fNode[0] = element;
